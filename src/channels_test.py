...
def test_channels_list():
    ...
    assert list_result['channels'][0]['name'] == 'test_channel'
    ...
    assert list_result2['channels'][0]['channel_id'] == 2
    assert list_result2['channels'][0]['name'] == 'test_channel_two'
    assert list_result2['channels'][1]['channel_id'] == 3
    assert list_result2['channels'][1]['name'] == 'test_channel_three'
...
def test_channels_create_name_valid():
    ...
    list_result = channels.channels_list(user['token'])
    assert list_result['channels'][0]['channel_id'] == 1
    assert len(list_result['channels'][0]['name']) < 20

    user2 = auth.auth_register('mailtest@gmail.com', 'passwordword', 'first_name', 'last_name')
    channels.channels_create(user['token'], 'test_name_two', False)
    list_result2 = channels.channels_list(user['token'])
    assert list_result2['channels'][0]['channel_id'] == 2
    assert len(list_result2['channels'][0]['name']) < 20
